...
class ReflexAgentWithState(Agent):
    def __init__(self, program):
        ...
        self.state = {'location': None, 'status': None}  # Initialize agent's state
        self.previous_action = None

def ReflexAgentWithStateProgram():
    """Reflex agent program with internal state."""
    def program(percept, agent):
        status, bump = percept
        if status == 'Dirty':
            print("Agent chose to suck")
            return 'Suck'
        elif bump == 'Bump':
            # Handle bump condition if necessary
            print("Agent bumped with a wall")
            pass
        else:
            # Update agent's internal state
            agent.state['location'] = agent.location
            agent.state['status'] = status
            
            # Decide action based on state
            # For example, if agent knows it's clean and last action was 'right', go 'down'
            if agent.state['status'] == 'Clean' and agent.previous_action == 'right':
                action = random.choice(['right', 'up', 'down'])
            elif agent.state['status'] == 'Clean' and agent.previous_action == 'left':
                action = random.choice(['left', 'up', 'down'])
            elif agent.state['status'] == 'Clean' and agent.previous_action == 'up':
                action = random.choice(['left', 'up', 'right'])
            elif agent.state['status'] == 'Clean' and agent.previous_action == 'down':
                action = random.choice(['left', 'down', 'right'])
            else: 
                action = random.choice(['right', 'left', 'up', 'down'])
                
            print("Agent chose to move", action)
            agent.previous_action = action  # Store previous action for next iteration
            return action
...
if __name__ == "__main__":
    ...
    reflex_agent_with_state = ReflexAgentWithState(program)

    # Checks the initial state of the environment
    vac_env.add_thing(reflex_agent_with_state)

    for i in range(100):
        print("Iteration:", i+1)
        
        # Check if the current location is dirty or clean
        status, _ = vac_env.percept(reflex_agent_with_state)
        if status == "Dirty":
            dirty_count += 1
        elif status == "Clean":
            clean_count += 1        
    ...
    print("Agent's performance score:", reflex_agent_with_state.performance)